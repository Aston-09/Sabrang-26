...
export async function POST(req: Request) {
  try {
    ...
    const regQuery = await adminDb.collection('registrations').where('qrCode', '==', qrCode).get();
    ...
    const userDocRef = await adminDb.collection('users').doc(regData.userId).get();
    const eventDocRef = await adminDb.collection('events').doc(regData.eventId).get();
